...
def save_admin_note(title, subject, program, board,
    ...
    try:
        ...
        cursor.execute("""
            ...
            (title, subject, program, board, year,
             topic, description, file_path,
        ...
        print(f"save_admin_note error: {e}")
        return False


def get_admin_notes(program=None, subject=None,
                    board=None, topic=None):
    """Get notes with optional filters"""
    conn = get_connection()
    cursor = conn.cursor()
    query  = "SELECT * FROM admin_notes WHERE 1=1"
    params = []
    if program:
...
def increment_note_downloads(note_id):
