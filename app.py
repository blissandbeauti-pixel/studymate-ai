<<<<<<< HEAD
# ============================================
# StudyMate AI - Main Application v7.0
# ============================================
...
try:
    os.environ["AI_BACKEND"]  = st.secrets.get("AI_BACKEND",  "groq")
...
def show_sidebar():
    with st.sidebar:
        ...
        # ── Mobile Install ──
        st.markdown("### 📱 Install on Mobile")
        st.markdown("""
            <div style='background:white; border-radius:10px;
            padding:12px 15px; margin:8px 0; font-size:12px;
            color:#333; line-height:1.8;
            border-left:4px solid #4CAF50;
            box-shadow:0 1px 4px rgba(0,0,0,0.08)'>

            <b style='color:#2E7D32'>📱 Install on Android:</b><br>
            1. Open app in Chrome browser<br>
            2. Tap menu (⋮) top right<br>
            3. Tap <b>"Add to Home Screen"</b><br>
            4. Tap <b>"Add"</b> — done! ✅<br><br>

            <b style='color:#1565C0'>🍎 Install on iPhone:</b><br>
            1. Open app in Safari browser<br>
            2. Tap Share button (□↑)<br>
            3. Tap <b>"Add to Home Screen"</b><br>
            4. Tap <b>"Add"</b> — done! ✅<br><br>

            <b style='color:#7B1FA2'>💻 Install on PC/Laptop:</b><br>
            1. Open app in Chrome browser<br>
            2. Click install icon (⊕) in address bar<br>
            3. Click <b>"Install"</b> — done! ✅
...
def generate_mcqs(exam_mode, program, year, institution,
    ...
    #____Prompt for regular vs entry test modes____
    if exam_mode == "Entry Test Preparation":
        prompt = f"""
    You are a STRICT Pakistani entry test examiner for {institution}.

    Entry Test : {institution}
    Subject    : {subject}
    Topic      : {topic}
    Difficulty : {difficulty}

    DIFFICULTY RULES — STRICTLY FOLLOW:
    - Easy   : Direct recall, basic definitions, single concept
    - Medium : Application-based, 2-step reasoning required
    - Hard   : Analytical, clinical/technical application,
               common misconceptions as wrong options,
               requires deep understanding NOT memorization
    - Advanced: PhD/specialist level, research-based,
                all options appear correct to average student,
                only deep conceptual understanding gives answer

    Generate exactly {num_questions} MCQs at {difficulty} level.

    STRICT RULES FOR {difficulty} DIFFICULTY:
    {"- Wrong options must look very convincing" if difficulty in ["Hard", "Advanced"] else ""}
    {"- No straightforward recall questions allowed" if difficulty in ["Hard", "Advanced"] else ""}
    {"- Questions must require analysis, not just memory" if difficulty in ["Hard", "Advanced"] else ""}
    {"- Base on real past paper patterns of " + institution if difficulty in ["Hard", "Advanced"] else ""}
    {"- Include clinical/technical application scenarios" if difficulty in ["Hard", "Advanced"] else ""}

    Format EACH MCQ exactly like this:

    Q1. [Question — must match {difficulty} level strictly]
    A) [Option — plausible]
    B) [Option — plausible]
    C) [Option — plausible]
    D) [Option — plausible]
    ...
    Key Note: [Critical exam point]
    Exam Tip: [Strategy for this question type]

    ---
    Generate {num_questions} questions. All must be {difficulty} level.
    Reject any question that seems too easy for {difficulty} standard.
    """
    else:
        prompt = f"""
    You are a STRICT Pakistani curriculum examiner.
    ...
    DIFFICULTY RULES — STRICTLY FOLLOW:
    - Easy   : Basic recall, definitions, single facts
    - Medium : Understanding + application required
    - Hard   : Analysis, evaluation, multi-concept questions
    - Advanced: Synthesis level, expert knowledge required,
                all distractors highly plausible

    STRICT RULES FOR {difficulty} DIFFICULTY:
    {"- Every wrong option must seem correct to unprepared student" if difficulty in ["Hard", "Advanced"] else ""}
    {"- Questions must test understanding, NOT memorization" if difficulty in ["Hard", "Advanced"] else ""}
    {"- Include scenario-based questions" if difficulty in ["Hard", "Advanced"] else ""}
    {"- Follow Bloom's Taxonomy Analysis/Evaluation level" if difficulty in ["Hard", "Advanced"] else ""}

    Generate exactly {num_questions} MCQs following
    Pakistani HEC curriculum for {institution}.
    ...
    Correct Answer: [Letter]) [Answer text]
    Explanation: [Why correct AND why others are wrong]
    Key Note: [Important exam point]

    ---
    All {num_questions} questions must strictly match
    {difficulty} difficulty. No easy questions allowed
    when {difficulty} is Hard or Advanced.
    """
...
def generate_mcqs_in_batches(exam_mode, program, year, institution,
                              subject, topic, num_questions, difficulty):
    """Generate large MCQ sets in batches of 50"""
    from ai_backend import chat
    ...
    for batch_num in range(total_batches):
        ...
        progress.progress(
            ...
            text=f"⏳ Batch {batch_num + 1}/{total_batches} "
                 f"({batch_num * batch_size}/{num_questions} done)..."
        )

        if exam_mode == "Entry Test Preparation":
            prompt = f"""
            You are an expert Pakistani entry test educator.
            Entry Test : {institution}
            Subject    : {subject}
            Topic      : {topic}
            Difficulty : {difficulty}
            Batch      : {batch_num + 1} of {total_batches}

            Generate exactly {current_batch} MCQs.
            Start numbering from Q{q_counter}.
            Avoid repeating concepts from previous batches.
            ...
            Key Note: [One exam point]
            Exam Tip: [One shortcut]

            ---
            """
        else:
            prompt = f"""
            You are an expert Pakistani curriculum educator.
        ...
        # ✅ This is now correctly inside the loop
        result = chat(prompt)
    ...
    if st.button("🚀 Generate MCQs", use_container_width=True):
        ...
        if errors:
            ...
            save_mcq_history(
                st.session_state.user["id"],
                exam_mode, program, year, institution,
                subject, topic, difficulty, num_questions, result
            )

            st.session_state.current_mcqs = result
            st.session_state.parsed_mcqs = parse_mcqs(result)
            st.session_state.current_meta = {
                "exam_mode": exam_mode, "program": program,
            ...
            st.success(
                f"✅ {len(st.session_state.parsed_mcqs)} MCQs Generated "
                f"& Saved!"
            )
...
def show_sidebar():
    with st.sidebar:
        ...
        st.markdown("""
        <div style='background:white; border-radius:10px;
        padding:14px 15px; margin:8px 0; font-size:12px;
        color:#333; line-height:1.8; border-left:4px solid #1565C0;
        box-shadow: 0 1px 4px rgba(0,0,0,0.08)'>

        <b style='color:#1565C0; font-size:13px'>
        🙏 A Humble Request</b><br><br>

        Assalam-o-Alaikum,<br><br>

        <b>StudyMate AI</b> is a free service built with the sincere
        intention of helping Pakistani students study smarter,
        prepare better, and achieve their dreams — whether it's
        clearing <b>MDCAT, ECAT</b>, excelling in
        <b>BSN, DAE, FSc</b> or any other program.<br><br>

        This tool is developed and maintained
        <b>entirely by one person</b>, in personal time,
        using personal resources. Every new feature —
        MCQ generation, smart notes, PDF export,
        assessment tracking — takes significant time,
        effort and dedication.<br><br>

        If <b>StudyMate AI</b> has helped you in your studies,
        even in a small way, I humbly request your support.
        Your contribution — no matter how small — directly
        helps in:<br><br>

        &nbsp;&nbsp;✅ Keeping the app <b>free for all students</b><br>
        &nbsp;&nbsp;✅ Adding new features regularly<br>
        &nbsp;&nbsp;✅ Supporting more boards, universities & programs<br>
        &nbsp;&nbsp;✅ Improving AI quality and speed<br>
        &nbsp;&nbsp;✅ Building more <b>free educational tools</b>
        for Pakistan<br><br>

        Even a small donation of <b>Rs. 50 or Rs. 100</b> makes
        a real difference and keeps this mission going.<br><br>
        ...
        st.markdown(
            ...
            "color:gray'>💳 <b>Support via</b></div>",
            unsafe_allow_html=True
        )
        st.write("")
        ...
        color:white; border-radius:10px; padding:12px 15px;
        margin:5px 0; text-align:center;
        box-shadow:0 2px 6px rgba(0,166,81,0.3)'>
            <div style='font-size:13px; font-weight:bold;
            letter-spacing:0.5px'>📱 EasyPaisa</div>
        ...
        box-shadow:0 2px 6px rgba(108,45,199,0.3)'>
            <div style='font-size:13px; font-weight:bold;
            letter-spacing:0.5px'>💜 SadaPay</div>
            <div style='font-size:18px; font-weight:bold;
            letter-spacing:2px; margin:4px 0'>
            0308-1816669</div>
            <div style='font-size:11px; opacity:0.9'>
        ...
        box-shadow:0 2px 6px rgba(255,107,53,0.3)'>
            <div style='font-size:13px; font-weight:bold;
            letter-spacing:0.5px'>🟠 NayaPay</div>
            <div style='font-size:18px; font-weight:bold;
            letter-spacing:2px; margin:4px 0'>
            0308-1816669</div>
            <div style='font-size:11px; opacity:0.9'>
        ...
        color:gray; line-height:1.8'>
            📧 Contact: your@email.com<br>
            © 2025 StudyMate AI v7.0<br>
...
def generate_mcqs(exam_mode, program, year, institution,
    ...
    if exam_mode == "Entry Test Preparation":
        prompt = f"""
        You are an expert Pakistani entry test preparation educator.
        Entry Test : {institution}
        Subject    : {subject}
        Topic      : {topic}
        Difficulty : {difficulty}

        Generate exactly {num_questions} MCQs strictly following
        {institution} past paper pattern.
        ...
        Key Note: [One important exam point]
        Exam Tip: [One trick or shortcut]

        ---
        Rules: No repeated concepts. Match {difficulty} difficulty.
        """
    else:
        prompt = f"""
        You are an expert Pakistani curriculum educator.
...
def generate_mcqs_in_batches(exam_mode, program, year, institution,
    ...
    all_content.append(response["message"]["content"])
    q_counter += current_batch
